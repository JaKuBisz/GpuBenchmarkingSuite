...
public class MatrixMultiplyParallel : IBenchmark
{
    private readonly int _threadCount;
    private int _size;
    private float[]? _a, _b, _c;

    public string Name => $"MatrixMultiply_Parallel_{_threadCount}";

    public MatrixMultiplyParallel(int threadCount)
    {
    ...
    public void Run()
    {
        ...
        System.Threading.Tasks.Parallel.For(0, n, options, i =>
        {
            for (int j = 0; j < n; j++)
            {
                float sum = 0f;
